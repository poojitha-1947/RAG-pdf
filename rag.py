...
def main():
    ...
    if pdf_file is not None:
        ...
        faiss_index = build_faiss_index(documents, context_encoder, context_tokenizer)
        ...
        if question:
            ...
            relevant_documents = retrieve_documents(question, faiss_index, documents, question_encoder, question_tokenizer, k=3)
            context = " ".join(relevant_documents)

            # Generate an answer based on the context and the question
            answer = generate_answer(question, context, bart_model, bart_tokenizer)
